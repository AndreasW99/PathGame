...
public class MapCell extends CellComponent {
	...
	private boolean isJewel; // Is this a jewel cell?
	...
	public static enum CellType {
		BLOCK, INITIAL, JEWEL, CROSS_PATH, INITIAL_PROCESSED, JEWEL_PROCESSED, INITIAL_POPPED, IN_STACK, OUT_STACK, HORIZ_PATH, VERT_PATH
	};
	
	
	public MapCell (CellType cType, int delay) {
		type = cType;
		originalType = cType;
		timeDelay = delay;
		this.isStart = (cType == CellType.INITIAL);
		this.isJewel = (cType == CellType.JEWEL);
	...
	private void setColor(CellType t) {
		...
		case INITIAL:
			...
			break;
		case JEWEL:
			this.setBackground(CellColours.JEWEL);
			break;
		case CROSS_PATH:
			this.setBackground(CellColours.OMNI_SWITCH);
			break;
		case JEWEL_PROCESSED:
			this.setBackground(CellColours.JEWEL_PROCESSED);
			break;
		...
		case IN_STACK:
			if (originalType == CellType.VERT_PATH)
				...
				this.setBackground(CellColours.INITIAL_PROCESSED);
			else if (originalType == CellType.JEWEL)
				this.setBackground(CellColours.JEWEL_PROCESSED);
			else
		...
		case OUT_STACK:
			if (originalType == CellType.VERT_PATH)
				...
				setBackground(CellColours.JEWEL_POPPED);
			else
	 ...
	 */
	public void markJewel() {
		this.type = CellType.JEWEL_PROCESSED;
		this.setColor(this.type);
		reDraw();
	}
